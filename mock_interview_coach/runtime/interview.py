...
from mock_interview_coach.agents._common import _LIMITS, _normalize, _truncate
...
class InterviewSession:
    ...
    def __init__(
        ...
        meta: dict[str, Any] = {}
        self.persona = self.agents["build_persona"](role, self.state, config=config_obj, meta=meta)
        self._log_call("build_persona", meta)
    ...
    def next_question(self) -> dict[str, Any]:
        ...
        self._log_call("ask_question", meta)
    ...
    def submit_answer(self, answer_text: str) -> dict[str, Any]:
        ...
        self.state.record_answer(self._sanitize_answer(answer_text))
    ...
    def submit_probe_answer(self, answer_text: str) -> dict[str, Any]:
        ...
        self.state.record_answer(
            self._sanitize_answer(answer_text), probe=self._active_probe
        )
    ...
    def finish(self) -> tuple[dict[str, Any], str]:
        ...
        if self._report is None:
            ...
            self._log_call("write_coach_narrative", meta)
            self._report = (summary, narrative)
        return self._report

    def _log_call(self, agent: str, meta: dict[str, Any]) -> None:
    ...
    def _evaluate_and_dispatch(self) -> dict[str, Any]:
        ...
        self._log_call("evaluate_answer", meta)
        provenance = {
            "model": meta.get("model"),
            "fallback": bool(meta.get("fallback")),
            "validated": bool(evaluation.get("validated", True)),
            "caps_applied": list(evaluation.get("caps_applied", [])),
        }
        self.state.record_evaluation(evaluation, provenance=provenance)
